...
describe("register routes", () => {
  it("registers a device through the GET compatibility endpoint", async () => {
    ...
    const response = await app.request("http://example.com/register?devicetoken=device-token-1");
  ...
  it("registers a device through a multipart form", async () => {
    const { app, registry } = createHarness();
    registry.setGeneratedKeys(["generated-multipart"]);
    ...
    await expect(response.json()).resolves.toMatchObject({
      code: 200,
      data: {
        device_key: "generated-multipart",
        device_token: "multipart-token",
      },
    });
    expect(registry.snapshot()).toEqual({
      "generated-multipart": "multipart-token",
    });
  });

  it("supports legacy key aliases", async () => {
    const { app, registry } = createHarness();
    registry.setGeneratedKeys(["generated-legacy"]);
    ...
    await expect(response.json()).resolves.toEqual({
      ...
      timestamp: 1_717_900_000,
      data: {
        key: "generated-legacy",
        device_key: "generated-legacy",
  ...
  it("does not reserve an unknown caller-supplied device key", async () => {
    const { app, registry } = createHarness();
    registry.setGeneratedKeys(["generated-for-long-alias"]);
    ...
    await expect(response.json()).resolves.toMatchObject({
      code: 200,
      data: {
        device_key: "generated-for-long-alias",
      },
    });
    expect(registry.snapshot()).toEqual({
      "generated-for-long-alias": "device-token",
    });
  });
  ...
  it("returns 400 when a checked device key does not exist", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      code: 400,
      message: "failed to get [missing] device token from database",
    });
  });

  it("returns 403 when registration is closed (POST)", async () => {
    const { app } = createHarness({ config: { registrationMode: "closed" } });
  ...
  it("allows known keys to refresh tokens in existing-only mode", async () => {
    const { app, registry } = createHarness({
      config: { registrationMode: "existing-only" },
      registrySeed: { alpha: "old-token" },
    });

    const response = await app.request(
      "http://example.com/register?key=alpha&devicetoken=new-token",
    );
  ...
  it("rejects unknown keys in existing-only mode", async () => {
    const { app, registry } = createHarness({
      config: { registrationMode: "existing-only" },
    });

    const response = await app.request(
      "http://example.com/register?key=unknown&devicetoken=new-token",
    );

    expect(response.status).toBe(403);
    await expect(response.json()).resolves.toMatchObject({
      code: 403,
      message: "new device registration is disabled",
    });
    expect(registry.snapshot()).toEqual({});
  });

  it("rejects new registrations after the configured device cap", async () => {
    const { app, registry } = createHarness({
      config: { maxDevices: 1 },
      registrySeed: { alpha: "token-alpha" },
    });

    const response = await app.request("http://example.com/register?devicetoken=new-token");
    ...
    expect(registry.snapshot()).toEqual({ alpha: "token-alpha" });
  });
