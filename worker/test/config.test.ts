...
describe("parseApnsRequestTimeoutMs", () => {
  ...
  it("accepts the hard maximum and rejects larger timeouts", () => {
    ...
    expect(parseApnsRequestTimeoutMs(String(HARD_MAX_APNS_REQUEST_TIMEOUT_MS + 1))).toBe(
      DEFAULT_APNS_REQUEST_TIMEOUT_MS,
    );
  });
...
describe("parseMaxRequestBodyBytes", () => {
  it("accepts the hard maximum and rejects larger bodies", () => {
    ...
    expect(parseMaxRequestBodyBytes(String(HARD_MAX_REQUEST_BODY_BYTES + 1))).toBe(
...
describe("parseRegistrationMode", () => {
  it("defaults to open", () => {
    expect(parseRegistrationMode()).toBe("open");
  });

  it("accepts existing-only and closed case-insensitively", () => {
    expect(parseRegistrationMode("existing-only")).toBe("existing-only");
    expect(parseRegistrationMode(" CLOSED ")).toBe("closed");
  });

  it("falls back to open for unknown values", () => {
    expect(parseRegistrationMode("false")).toBe("open");
    expect(parseRegistrationMode("unknown")).toBe("open");
  });
});

describe("parseMaxDevices", () => {
  it("uses the default when absent or invalid", () => {
    expect(parseMaxDevices()).toBe(DEFAULT_MAX_DEVICES);
    expect(parseMaxDevices("0")).toBe(DEFAULT_MAX_DEVICES);
    expect(parseMaxDevices("invalid")).toBe(DEFAULT_MAX_DEVICES);
  });

  it("accepts the hard maximum and rejects larger values", () => {
    expect(parseMaxDevices(String(HARD_MAX_DEVICES))).toBe(HARD_MAX_DEVICES);
    expect(parseMaxDevices(String(HARD_MAX_DEVICES + 1))).toBe(DEFAULT_MAX_DEVICES);
  });
});
