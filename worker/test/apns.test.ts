...
AA==
-----END PRIVATE KEY-----`;
const TEST_PKCS8_PRIVATE_KEY_SINGLE_LINE =
  "-----BEGIN PRIVATE KEY-----AA==-----END PRIVATE KEY-----";
const TEST_PKCS8_PRIVATE_KEY_ESCAPED =
  "-----BEGIN PRIVATE KEY-----\\nAA==\\n-----END PRIVATE KEY-----";
...
describe("CloudflareApnsClient", () => {
  ...
  it("reuses APNs provider JWTs within the refresh window", async () => {
    ...
    await client.send(createMessage());
    now.mockReturnValue(1_700_000_000_000 + 49 * 60 * 1000);
    await client.send(createMessage());
    now.mockReturnValue(1_700_000_000_000 + 50 * 60 * 1000);
    await client.send(createMessage());
  ...
  it("rejects payloads larger than the APNs 4096-byte limit before fetch", async () => {
    installCryptoStub();
    const client = new CloudflareApnsClient({
      privateKey: TEST_PKCS8_PRIVATE_KEY,
      keyId: "KEYID123",
    ...
    await expect(client.send(createMessage({ body: "x".repeat(5000) }))).rejects.toMatchObject({
  ...
  it("refreshes an expired provider token and retries exactly once", async () => {
    ...
    await expect(client.send(createMessage())).resolves.toBeUndefined();
