...
describe("mcp compatibility", () => {
  ...
  it("handles push failure", async () => {
    ...
    harness.sender.failForDeviceToken("bad-token", createApnsError("BadDeviceToken", 400));
  ...
  it("valid session allows subsequent requests", async () => {
    ...
    const res = await jsonRpcRequest(app, "/mcp", "tools/list", undefined, 1, {
  ...
  it("/mcp/:device_key session cannot access different device_key", async () => {
    ...
    const res = await jsonRpcRequest(app, "/mcp/key-b", "tools/list", undefined, 1, {
      "mcp-session-id": sessionId,
    });
  ...
  it("expired session returns 404", async () => {
    ...
    const res = await jsonRpcRequest(harness2.app, "/mcp", "tools/list", undefined, 1, {
      "mcp-session-id": sessionId,
    });
