...
describe("basic auth compatibility", () => {
  ...
  it("allows auth-free endpoints without credentials", async () => {
    ...
    await expect(app.request("http://example.com/healthz")).resolves.toMatchObject({ status: 200 });
    await expect(app.request("http://example.com/register?devicetoken=abc")).resolves.toMatchObject(
      { status: 200 },
    );
