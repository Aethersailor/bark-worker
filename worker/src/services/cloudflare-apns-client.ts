...
const APNS_PAYLOAD_MAXIMUM_BYTES = 4096;
...
function parsePemPrivateKey(pem: string): ArrayBuffer {
  ...
  const match = normalizedPem.match(/-----BEGIN [^-]+-----\s*([\s\S]*?)\s*-----END [^-]+-----/);
...
export class CloudflareApnsClient implements PushSender {
  ...
  async send(message: PushMessage): Promise<void> {
    ...
    if (new TextEncoder().encode(payloadText).byteLength > APNS_PAYLOAD_MAXIMUM_BYTES) {
      const error = new Error("PayloadTooLarge") as ApnsSendError;
      error.statusCode = 413;
      error.reason = "PayloadTooLarge";
      throw error;
    }
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        response = await fetch(url, {
          ...
          headers,
          body: payloadText,
          signal: AbortSignal.timeout(this.config.timeoutMs ?? DEFAULT_APNS_REQUEST_TIMEOUT_MS),
        });
      } catch (networkError) {
        const error = new Error(
          `APNs network error: ${networkError instanceof Error ? networkError.message : String(networkError)}`,
        ) as ApnsSendError;
        error.statusCode = 500;
      ...
      if (attempt === 0 && response.status === 403 && reason === "ExpiredProviderToken") {
        this.cachedJwt = null;
        continue;
      }
    ...
    throw new Error("APNs request exhausted retries");
  }
}
