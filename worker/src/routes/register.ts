...
async function doRegister(c: Context, options: RegisterRouteOptions, compat: boolean) {
  if (options.config.registrationMode === "closed") {
  ...
  const deviceTokenValue = preferPrimaryField(deviceInfo.device_token, deviceInfo.devicetoken);
  ...
  try {
    const newKey = await options.deps.registry.saveDeviceTokenByKey(deviceKey, deviceToken, {
      allowNew: options.config.registrationMode === "open",
      maxDevices: options.config.maxDevices,
    });
    return c.json(
    ...
    if (error instanceof RegistrationRejectedError) {
      return c.json(failed(options.deps.now(), 403, error.message), 403);
    }
    console.error(
      JSON.stringify({
