...
const ACCEPTED_PROTOCOL_VERSIONS = ["2025-03-26", "2025-06-18", "2025-11-25"] as const;
...
const LEGACY_INITIALIZE_PROTOCOL_VERSION_SET = new Set<string>(LEGACY_INITIALIZE_PROTOCOL_VERSIONS);
...
type SessionValidationResult =
  { valid: true; payload: SessionPayload } | { valid: false; status: 400 | 404; message: string };

type ProtocolValidationResult =
  { valid: true; version: string | null } | { valid: false; version: string };

type InitializeNegotiationResult =
  { valid: true; version: string } | { valid: false; version: string };
...
function isJsonRpcClientResponse(body: JsonRpcRequest): boolean {
  ...
  if (!("id" in body) || !isJsonRpcId(body.id) || "method" in body || hasResult === hasError) {
    return false;
  }
...
async function validateSessionToken(
  ...
  if (!(await timingSafeStringEqual(signatureB64, expectedSig))) {
...
function buildNotifyTool(deviceKeyRequired: boolean) {
  const properties: Record<string, unknown> = {
    ...
    isArchive: {
      type: "string",
      description: "Set to '1' to save the notification or any other value to skip saving",
    },
    ttl: {
      type: "number",
      description:
...
export function registerMcpRoutes(app: Hono, options: McpRouteOptions): void {
  async function handlePost(c: Context, pathDeviceKey: string | null) {
    ...
    if (body.method === "initialize") {
      ...
      const negotiation = negotiateInitializeProtocolVersion(clientVersion ?? protocolHeader);
      if (negotiation.valid) {
    ...
    if (
      ...
      const negotiatedVersion =
        protocolHeader ??
        negotiatedInitializeProtocolVersion ??
        (fallbackNegotiation.valid ? fallbackNegotiation.version : SERVER_PROTOCOL_VERSION);
  ...
  app.post("/mcp/:device_key", (c) => handlePost(c, c.req.param("device_key") ?? null));

  const methodNotAllowed = (c: Context) => {
    setProtocolHeader(c, responseProtocolVersionForHeader(c.req.header("mcp-protocol-version")));
